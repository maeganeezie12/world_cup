...
## 1. Overview
A lightweight, self-hosted website for tracking a friends/family/office World Cup guessing game, starting from the Quarterfinal round through the Final. It's a **private, invite-only game**: the whole site sits behind a shared access password (§4), so it isn't publicly reachable — only people you've given the password to can join. Once in, players join, pick match winners round by round, and see a live leaderboard. This is a private prediction/scorekeeping tool — it does not process, hold, or move money. Money changes hands off-platform, entirely by the players' own arrangement (honor system); the site is just the single source of truth for who picked what, who's paid, and who's owed what at the end.
...
- There's **no fixed $2 charged to everyone**. A player only owes the entry amount for the matches they actually have a chance to pick — i.e., matches that haven't kicked off yet at the moment they join.
  ...
  - And so on — a player's total buy-in is just the sum of the entry amounts for whichever matches are still open when they join (see §8 for entry amounts).
...
- If a player doesn't submit a pick before kickoff, they simply **sit out that match** — no entry, no winnings, no loss for that one.

## 8. Scoring & Payout
This is the only payout mechanism — no bonuses, multipliers, or point weighting on top of it.

| Match | Count | Entry per match | Subtotal |
|---|---|---|---|
...
- Every player who submits a pick for a match puts that match's entry amount into that match's pot.
- After the result is in, the pot is **split evenly among everyone who picked the winning team**.
- Players who picked wrong get nothing back from that match — their entry stays in the pot and funds the winners' split.
...
2. **Home / Leaderboard (Scoreboard)** — ranked by total winnings. Columns per player:
   ...
   - **Accuracy** — correct picks ÷ picks made (settled matches only), shown as a %.
   - **Picks made** — count of matches picked so far (out of the 8 total, or fewer if joined late).
   - Paid/unpaid status.
...
- `Match`: id, type (QF/SF/THIRD_PLACE/FINAL), entry_amount, team_a, team_b, kickoff_at, winner (nullable until result entered)
...
- Site is **not publicly discoverable/joinable** without the access password (§4) — reduces exposure as an open, publicly-reachable service; it's scoped to an invited private group, and framed throughout as a private prediction/scorekeeping tool rather than a betting platform.
...
- Bonuses, multipliers, or any payout weighting beyond the flat per-match entry amounts in §8.
